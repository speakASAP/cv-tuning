...
function errorMessage(error: unknown) {
  ...
  if (status === 504 || status === 502 || /<html[\s>]|Gateway time-?out|504:/i.test(raw)) {
    return 'The AI request took too long and was cut off. Try again with a shorter instruction.';
  }
  if (/AI_HTTP_TIMEOUT/i.test(raw)) {
    return 'The AI model did not finish in time. Try again with a shorter instruction.';
  }
  try {
    const body = JSON.parse(raw) as { message?: string | string[] };
    return Array.isArray(body.message) ? body.message.join(', ') : body.message || raw;
  } catch { return raw; }
}
...
function App() {
  ...
  /** Per-action progress text. Global busy was deleted — one long AI call must not freeze tabs. */
  const [busy, setBusy] = useState<Record<string, string>>({});
  const busyRef = useRef<Record<string, string>>({});
  ...
  const run = async (key: string, work: () => Promise<void>, working = 'Working on it, please wait...') => {
    if (busyRef.current[key]) return;
    busyRef.current = { ...busyRef.current, [key]: working };
    setBusy(busyRef.current);
    setError('');
    try { await work(); } catch (cause) { setError(errorMessage(cause)); } finally {
  ...
  const loadApplication = async (application: Row) => {
    ...
    else { setRevision(null); setManualMarkdown(''); setDiff(null); }
  };
  ...
  useEffect(() => {
    if (!token) return;
    run('bootstrap', async () => { await loadWorkspace(); await loadApplications(); }, 'Loading your workspace...');
  }, [token]);
  useEffect(() => {
    if (!token || tab !== 'dashboard') return;
    run('dashboard', async () => setDashboard(await api<Row>('/api/dashboard', token)), 'Loading your dashboard...');
  }, [tab, token]);

  const grantConsent = () => run('consent', async () => { await api('/api/master/consent', token, { method: 'POST' }); setConsent(true); }, 'Recording your consent...');
  const saveMaster = (event: FormEvent) => { event.preventDefault(); run('save-master', async () => { await api('/api/master', token, { method: 'POST', body: JSON.stringify({ markdown }) }); await loadWorkspace(); }, 'Saving your master CV and extracting its facts...'); };
  const uploadMaster = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = event.currentTarget; const file = new FormData(form).get('file'); if (!(file instanceof File) || !file.size) { setError('Choose a CV file first.'); return; } run('upload-master', async () => { await api('/api/master/import/upload', token, { method: 'POST', body: new FormData(form) }); setSelectedFile(false); form.reset(); await loadWorkspace(); }, 'File received. Reading the document and extracting its facts. A scan is recognised with OCR, which takes longer.'); };
  const importGoogleDoc = (event: FormEvent) => { event.preventDefault(); run('import-gdocs', async () => { await api('/api/master/import/gdocs', token, { method: 'POST', body: JSON.stringify({ url: gdocsUrl }) }); setGdocsUrl(''); await loadWorkspace(); }, 'Link received. Fetching your Google Doc and extracting its facts, please wait...'); };
  const addJob = (event: FormEvent) => { event.preventDefault(); run('add-job', async () => {
  ...
  const createApplication = (jobId: string) => run(`create-app:${jobId}`, async () => { await api('/api/applications', token, { method: 'POST', body: JSON.stringify({ jobId }) }); await loadApplications(); setTab('applications'); }, 'Building your tailored CV: matching your facts to the requirements and checking every claim. This takes a minute or two...');
  const decide = (bulletId: string, decision: 'confirm' | 'drop') => run(`decide:${bulletId}`, async () => {
    if (!selected || !revision) return;
    const applicationId = String(selected.id);
    await api(`/api/applications/${applicationId}/renders/${revision.revisionNo}/confirm-claim`, token, { method: 'POST', body: JSON.stringify({ bulletId, decision }) });
    if (selectedRef.current && String(selectedRef.current.id) === applicationId) await loadApplication(selectedRef.current);
  }, 'Recording your decision and re-checking the claim...');
  const revise = (event: FormEvent) => { event.preventDefault(); run('revise', async () => {
    if (!selected) return;
    const applicationId = String(selected.id);
    await api(`/api/applications/${applicationId}/revise`, token, { method: 'POST', body: JSON.stringify({ instruction, inputMode: 'text' }) });
    setInstruction('');
    if (selectedRef.current && String(selectedRef.current.id) === applicationId) await loadApplication(selectedRef.current);
  }, 'Revising in your voice and re-checking every claim against your facts. This takes a minute...'); };
  const dictate = () => {
  ...
  const mutate = (key: string, path: string, body?: Row, working?: string) => run(key, async () => {
    if (!selected) return;
    const applicationId = String(selected.id);
    await api(`/api/applications/${applicationId}/${path}`, token, { method: 'POST', ...(body ? { body: JSON.stringify(body) } : {}) });
    await loadApplications();
    if (selectedRef.current && String(selectedRef.current.id) === applicationId) await loadApplication(selectedRef.current);
  }, working ?? 'Updating this application...');
  const saveManualEdit = () => run('save-edit', async () => {
    if (!selected) return;
    const applicationId = String(selected.id);
    await api(`/api/applications/${applicationId}/edit`, token, { method: 'POST', body: JSON.stringify({ markdown: manualMarkdown }) });
    await loadApplications();
    if (selectedRef.current && String(selectedRef.current.id) === applicationId) await loadApplication(selectedRef.current);
  }, 'Saving your manual CV edit...');
  const download = (kind: 'pdf' | 'docx') => run(`download-${kind}`, async () => {
    if (!selected || !revision) return;
    const response = await fetch(`/api/applications/${selected.id}/renders/${revision.revisionNo}/download/${kind}`, { headers: { authorization: `Bearer ${token}` } });
    if (!response.ok) throw new Error(await response.text());
    const url = URL.createObjectURL(await response.blob());
    const link = document.createElement('a');
    link.href = url; link.download = `cv-r${revision.revisionNo}.${kind}`; link.click(); URL.revokeObjectURL(url);
  ...
  const openApplication = (application: Row) => run(`open-app:${application.id}`, async () => { await loadApplication(application); setTab('applications'); }, 'Opening this application...');
  ...
  const BusyNote = ({ action }: { action: string }) => busy[action]
    ? <p className="notice working" role="status" aria-live="polite" aria-busy="true">{busy[action]}</p>
    : null;
  return <main className="app-shell"><header><div><p className="eyebrow">CV TUNING</p><h1>Evidence-first application workspace</h1></div><button className="secondary" onClick={logout}>Sign out</button></header><nav>{(['workspace', 'applications', 'dashboard'] as Tab[]).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item}</button>)}</nav>{error && <p className="notice error error-banner" role="alert">{error}</p>}
  {tab === 'workspace' && <section className="grid"><article className="panel"><h2>1. Consent and master CV</h2><BusyNote action="bootstrap" />{consent === false && <div className="notice"><p>AI processing requires your explicit consent.</p><BusyNote action="consent" /><button disabled={isBusy('consent')} onClick={grantConsent}>Grant CV-processing consent</button></div>}{consent && <><form onSubmit={saveMaster}><p className="muted">{masterVersion ? `Current master CV: version ${masterVersion}.` : 'Paste your first master CV.'}</p><label>Master CV<textarea rows={14} required value={markdown} onChange={(event) => setMarkdown(event.target.value)} placeholder="# Your name\n\n## Experience..." /></label><BusyNote action="save-master" /><button disabled={isBusy('save-master') || !markdown.trim()}>Save and extract facts</button></form><form className="compact-form" onSubmit={importGoogleDoc}><label>Import from Google Docs URL<input type="url" required value={gdocsUrl} onChange={(event) => setGdocsUrl(event.target.value)} placeholder="https://docs.google.com/document/d/..." /></label><p className="field-help">In Google Docs, choose <strong>Share → Anyone with the link → Viewer</strong>. The document is imported as your master CV.</p><BusyNote action="import-gdocs" /><button disabled={isBusy('import-gdocs') || !gdocsUrl.trim()}>Import Google Doc</button></form><form className="compact-form" onSubmit={uploadMaster}><label>Or import PDF, DOCX, text, a photo or scan of your CV, or a LinkedIn archive<input name="file" type="file" accept=".pdf,.docx,.txt,.zip,.png,.jpg,.jpeg,.tiff,.webp" onChange={(event) => setSelectedFile(Boolean(event.target.files?.length))} /></label><BusyNote action="upload-master" /><button disabled={isBusy('upload-master') || !selectedFile}>Upload and extract</button></form></>}</article><article className="panel"><h2>2. Add a job</h2><form onSubmit={addJob}><label>Job posting URL<input type="url" value={jobUrl} onChange={(event) => setJobUrl(event.target.value)} placeholder="https://company.example/careers/role" /></label><label>Paste the posting<textarea rows={8} value={jobText} onChange={(event) => setJobText(event.target.value)} placeholder="Paste it here when automatic retrieval is blocked..." /></label><BusyNote action="add-job" /><button disabled={isBusy('add-job') || (!jobUrl.trim() && !jobText.trim())}>Add job</button></form><h3>Saved jobs</h3><div className="list">{jobs.length ? jobs.map((job) => { const action = jobAction(job); const jobKey = action.creates ? `create-app:${job.id}` : `open-app:${applications.find((application) => String(application.jobId) === String(job.id))?.id ?? job.id}`; return <div className="row" key={String(job.id)}><div><strong>{String(job.title ?? 'Untitled position')}</strong><span>{String(job.company ?? job.fetchStatus ?? '')}</span>{job.fetchStatus !== 'ok' && <span className="warning">Posting needs pasted text: {String(job.fetchReason ?? 'not retrievable')}</span>}{busy[jobKey] && <span className="notice working">{busy[jobKey]}</span>}</div><button disabled={isBusy(jobKey) || action.disabled} onClick={action.act}>{action.label}</button></div>; }) : <p className="muted">No jobs yet.</p>}</div></article></section>}
  {tab === 'applications' && <section className="applications-layout"><article className="panel application-list"><h2>Applications</h2>{applications.length ? applications.map((application) => { const openKey = `open-app:${application.id}`; return <button className={`application ${selected?.id === application.id ? 'selected' : ''}`} key={String(application.id)} disabled={isBusy(openKey)} onClick={() => run(openKey, () => loadApplication(application), 'Loading this application...')}><strong>{String(application.jobTitle ?? 'Untitled position')}</strong><span>{[application.jobCompany ? String(application.jobCompany) : '', stateLabel(String(application.state)), String(application.renderLanguage).toUpperCase() + ' · ' + Number(application.revisionCount) + ' revisions'].filter(Boolean).join(' · ')}</span>{Boolean(application.stateError) && <span className="error">{String(application.stateError)}</span>}{busy[openKey] && <span className="notice working">{busy[openKey]}</span>}</button>; }) : <p className="muted">Create an application from a saved job.</p>}</article><article className="panel review">{!selected ? <p className="muted">Choose an application to review its evidence and revisions.</p> : <><div className="review-head"><div>{Boolean(selected.jobTitle) ? <h2 className="cv-view-position">Applying for: {String(selected.jobTitle)}{selected.jobCompany ? ` · ${String(selected.jobCompany)}` : ''}</h2> : <h2>Review and approval</h2>}</div><span className="badge">{stateLabel(String(selected.state))}</span></div>{Boolean(revision?.degraded) && <p className="notice warning">Generated using fallback model {String(revision?.modelUsed)}. Review with extra care.</p>}{Boolean(selected.stateError) && (selected.state === 'approved' ? <div className="notice error"><p><strong>Export did not finish.</strong> Your CV is approved and its content is safe — only the downloadable files are missing. {String(selected.stateError)}</p><BusyNote action="retry-export" /><button disabled={isBusy('retry-export')} onClick={() => mutate('retry-export', 'retry-export', undefined, 'Retrying the export...')}>Retry export</button></div> : <p className="notice error">{String(selected.stateError)}</p>)}<div className={`review-grid${pendingOverreachBullets.length === 0 ? ' review-grid-single' : ''}`}><section><div className="cv-view-tabs"><button className={cvView === 'preview' ? 'active' : ''} onClick={() => setCvView('preview')}>Preview</button><button className={cvView === 'markdown' ? 'active' : ''} onClick={() => setCvView('markdown')}>Markdown</button><button className={cvView === 'diff' ? 'active' : ''} onClick={() => setCvView('diff')}>Diff from {diff?.baselineRevisionNo ? `revision ${diff.baselineRevisionNo}` : 'master CV'}</button></div>{cvView === 'preview' && <CvPreview markdown={String(revision?.markdown ?? '')} />}{cvView === 'markdown' && <div className="manual-markdown-editor"><label>CV markdown<textarea rows={20} value={manualMarkdown} onChange={(event) => setManualMarkdown(event.target.value)} /></label><p className="field-help">Manual edits are saved exactly as written. Keep the H1, H2, H3, and bullet structure so exports retain your intended layout.</p><BusyNote action="save-edit" /><button disabled={isBusy('save-edit') || !manualMarkdown.trim()} onClick={saveManualEdit}>Save edit</button></div>}{cvView === 'diff' && <DiffView hunks={hunks} />}</section>{pendingOverreachBullets.length > 0 && <section><h3>Claims needing your decision</h3><p className="muted">Every generated claim is checked against your master CV. Only a claim that reaches beyond it — an “overreach” — ever needs a decision here: confirm it if it is true, or drop it. Once you decide, it stops appearing.</p>{pendingOverreachBullets.map((bullet) => { const decideKey = `decide:${bullet.bulletId}`; return <div className="claim overreach" key={String(bullet.bulletId)}><p>{String(bullet.text)}</p><small>Source fact: {String(bullet.sourceFactId)}{bullet.targetRequirement ? ` · targets ${String(bullet.targetRequirement)}` : ''}</small><BusyNote action={decideKey} /><div><button disabled={isBusy(decideKey)} onClick={() => decide(String(bullet.bulletId), 'confirm')}>Confirm claim</button><button className="secondary" disabled={isBusy(decideKey)} onClick={() => decide(String(bullet.bulletId), 'drop')}>Drop claim</button></div></div>; })}</section>}</div><form className="revision-form" onSubmit={revise}><h3>Revise in your voice</h3><textarea required value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Ask for a change. Unsupported requests remain blocked by evidence checks." disabled={isBusy('revise')} /><BusyNote action="revise" /><div><button type="button" className="secondary mic-button" onClick={dictate} aria-label="Use microphone" disabled={isBusy('revise')}><svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true"><path fill="currentColor" d="M12 14a3 3 0 0 0 3-3V5a3 3 0 1 0-6 0v6a3 3 0 0 0 3 3Zm5-3a1 1 0 1 0-2 0 5 5 0 1 1-10 0 1 1 0 1 0-2 0 7 7 0 0 0 6 6.93V20H8a1 1 0 1 0 0 2h8a1 1 0 1 0 0-2h-3v-2.07A7 7 0 0 0 17 11Z"/></svg>Use microphone</button><button disabled={isBusy('revise')}>Create revision</button></div></form><div className="actions"><BusyNote action="regenerate" /><BusyNote action="approve" /><BusyNote action="download-pdf" /><BusyNote action="download-docx" /><BusyNote action="mark-sent" />{selected.state === 'generation_failed' && !revision && <button disabled={isBusy('regenerate')} onClick={() => mutate('regenerate', 'regenerate', undefined, 'Retrying generation. Matching your facts to the requirements again...')}>Retry generation</button>}<button disabled={isBusy('approve') || !revision || (selected.state !== 'in_review' && selected.state !== 'generation_failed')} onClick={() => mutate('approve', 'approve')}>Approve after review</button><button className="secondary" disabled={isBusy('download-pdf') || !revision} onClick={() => download('pdf')}>Download PDF</button><button className="secondary" disabled={isBusy('download-docx') || !revision} onClick={() => download('docx')}>Download DOCX</button><button className="secondary" disabled={isBusy('mark-sent')} onClick={() => mutate('mark-sent', 'mark-sent')}>Mark as sent</button></div><div className="supplements"><h3>Application supplements</h3><BusyNote action="cover-letter" /><button className="secondary" disabled={isBusy('cover-letter')} onClick={() => supplement('cover-letter')}>Generate cover letter</button><label>Screening questions (one per line)<textarea rows={3} value={questions} onChange={(event) => setQuestions(event.target.value)} disabled={isBusy('screening')} /></label><BusyNote action="screening" /><button className="secondary" disabled={isBusy('screening')} onClick={() => supplement('screening')}>Generate screening answers</button></div><div className="outcomes"><span>Outcome:</span><BusyNote action="outcome" />{['interview', 'offer', 'rejected', 'ghosted'].map((outcome) => <button className="secondary" disabled={isBusy('outcome')} key={outcome} onClick={() => mutate('outcome', 'outcome', { outcome })}>{outcome}</button>)}</div></>}</article></section>}
  {tab === 'dashboard' && <section className="panel"><h2>Application outcomes</h2>{isBusy('dashboard') && !dashboard && <p className="notice working" role="status" aria-live="polite" aria-busy="true">Loading your dashboard...</p>}{dashboard ? <div className="metrics">{Object.entries((dashboard.funnel as Record<string, number>) ?? {}).map(([label, value]) => <div className="metric" key={label}><span>{label.replace('_', ' ')}</span><strong>{String(value)}</strong></div>)}<div className="metric"><span>Interview rate</span><strong>{dashboard.interviewRate == null ? '—' : `${Math.round(Number(dashboard.interviewRate) * 100)}%`}</strong></div><div className="metric"><span>Median reply days</span><strong>{String(dashboard.medianReplyDays ?? '—')}</strong></div></div> : !isBusy('dashboard') && <p className="muted">No dashboard data yet.</p>}</section>}</main>;
}
